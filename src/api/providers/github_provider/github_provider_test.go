...
// Entry point para inicialização dos testes
...
func TestCreateRepoNoError(t *testing.T) {
	...
	restclient.AddMockup(restclient.Mock{
		...
		Response: &http.Response{
			StatusCode: http.StatusCreated,
			Body:       ioutil.NopCloser(strings.NewReader(`{"id": 279714693,"name": "golang-microservice-tutorial-2","full_name": "oldschool69/golang-microservice-tutorial-2"}`)),
		},
	})
	response, err := CreateRepo("", github.CreateRepoRequest{})
	assert.Nil(t, err)
	assert.NotNil(t, response)
	assert.EqualValues(t, 279714693, response.Id)
	assert.EqualValues(t, "golang-microservice-tutorial-2", response.Name)
